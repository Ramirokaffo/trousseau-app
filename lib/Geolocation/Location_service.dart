...
import '../models/ServiceAddress.dart';
...
class LocationService {
    ...
    await Geolocator.isLocationServiceEnabled()
      ...
      if (!serviceEnabled) {
        showUniversalSnackBar(
            context: context,
            message:
                "Le service de localisation n'est pas activé sur votre appareil");
        return null;
      } else {
        await Geolocator.checkPermission()
            .then((LocationPermission permissionGranted) async {
          ...
          if (permissionGranted == LocationPermission.denied) {
            await Geolocator.requestPermission()
              ...
              if (permissionGranted == LocationPermission.denied) {
                showUniversalSnackBar(
                    context: context,
                    message: "Nous n'avons pas pu déterminer votre position");
                return null;
              }
            });
  ...
  static Future<ServiceAddress> getUserAddress() async {
    print("Ici dedans");
    ServiceAddress deliveryAddress = ServiceAddress();
